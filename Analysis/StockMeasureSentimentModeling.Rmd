...
Again, the same methodology of timeslice cross-validation was used to train my kNN models in *doKNN*. Similar to the RF and XGB models, hyper-parameter tuning was performed to identify the model configuration which produces the best model. The parameter tuned in this case was *k*, representative of the number of neighbors to be used in model generation. As was done previously, R-squared, MSE, RMSE, MAE, and MAPE metrics were calculated for both cross-validated training data and prediction data, and visual R-squared evaluation plots and actual vs. predicted stock closing prices, differenced closing prices, percent returns, and differenced volatility were plotted to PDF for both training and prediction datasets (See *KNNplots_* prefixed PDFs). Finally, feature importance rankings were captured for later review in Tableau.

A key difference in how this model type was developed vs. the other models in my analysis is that I scaled all predictor and response variables during kNN model generation. This was necessary given kNN's assessment of node distance to construct its regression model. Though I've left all stock measure plots and predictions in their scaled form, they still depict the same patterns and characteristics as their unscaled originators, just with scaled values instead.
    ...
    tryCatch({
        ...
        if(xform == "diff"){
            ...
            plot(as.numeric(cumsum(c(pp.orig.pred.df[,orig.response][1], pp.pred.df[,response]))),
                 type = "l", lty = 1, xlab = "Date & Hour",
                 ylab = paste0("Cumulative Sum of ", orig.response, "[1] and DIff = ", orig.response),
                 xaxt = "n", ylim = c(min(c(cumsum(c(pp.orig.pred.df[,orig.response][1],
...
As was done for both RF and XGB model function wrappers, the above *doKNN* function was called via several combinations of ticker, response measure (closing price, differenced closing price, percent return, and differenced volatility), and R-squared vs. RMSE evaluation metrics. These function calls are as depicted below.
...
AAPL.closediff.knn.Rsquared <- doKNN(AAPL.train, AAPL.pred, "AAPL", "close.diff", close.diff ~ .,
                                     "Rsquared", "diff", AAPL.train.adder, AAPL.pred.adder, "close")
XOM.closediff.knn.Rsquared <- doKNN(XOM.train, XOM.pred, "XOM", "close.diff", close.diff ~ .,
                                    "Rsquared", "diff", XOM.train.adder, XOM.pred.adder, "close")
AAPL.closediff.knn.RMSE <- doKNN(AAPL.train, AAPL.pred, "AAPL", "close.diff", close.diff ~ .,
                                 "RMSE", "diff", AAPL.train.adder, AAPL.pred.adder, "close")
XOM.closediff.knn.RMSE <- doKNN(XOM.train, XOM.pred, "XOM", "close.diff", close.diff ~ .,
                                "RMSE", "diff", XOM.train.adder, XOM.pred.adder, "close")
...
AAPL.volatilitydiff.knn.Rsquared <- doKNN(AAPL.train, AAPL.pred, "AAPL", "volatility.diff", volatility.diff ~ .,
                                          "Rsquared", "diff", AAPL.train.adder, AAPL.pred.adder, "volatility")
XOM.volatilitydiff.knn.Rsquared <- doKNN(XOM.train, XOM.pred, "XOM", "volatility.diff", volatility.diff ~ .,
                                         "Rsquared", "diff", XOM.train.adder, XOM.pred.adder, "volatility")
AAPL.volatilitydiff.knn.RMSE <- doKNN(AAPL.train, AAPL.pred, "AAPL", "volatility.diff", volatility.diff ~ .,
                                      "RMSE", "diff", AAPL.train.adder, AAPL.pred.adder, "volatility")
XOM.volatilitydiff.knn.RMSE <- doKNN(XOM.train, XOM.pred, "XOM", "volatility.diff", volatility.diff ~ .,
                                     "RMSE", "diff", XOM.train.adder, XOM.pred.adder, "volatility")

print(Sys.time() - startOverall)
```

An example plotted output via the *doKNN* function calls is as follows. The example shown below is for the AAPL hourly closing price (To see all outputs, please refer to the *KNNplots_* PDF file outputs included with this project's submission).

![AAPL, Closing Price, Rsquared kNN Model Evaluation Output](KNNplots_AAPLclose_Rsquared.pdf){width=800px height=800px}

Though run time was much faster than my other models (~16 mins), k-Nearest-Neighbor models and other *doKNN* returned outputs were saved to RDS files after generation to avoid needing to re-run the code multiple times.
...
#### Generalized Additive Model with Regression Splines
For the final model type, GAM with Splines, the same methodology of timeslice cross-validation was used for training in *doGAM*. Similar to all other model types, hyper-parameter tuning was performed to identify the model configuration which produces the best model. The parameter tuned in this case was *df*, or the number of degrees of freedom to be defined. The same metrics of R-squared, MSE, RMSE, MAE, and MAPE metrics were calculated for both cross-validated training data and prediction data. Finally, visual R-squared evaluation plots and actual vs. predicted stock closing prices, differenced closing prices, percent returns, and differenced volatility were plotted to PDF for both training and prediction datasets (See *KNNplots_* prefixed PDFs), and feature importance rankings were captured for later review.

The reason for also implementing a GAM with Splines model was to try to overcome the poor performance of Project 1's multiple linear regression model. Since the GAM treats response relationships additively, the hope was that it could account for both linearities as well as non-linearities in the stock measure data. Unfortunately, as is described in the paper write-up included with this project's submission, the GAM model proved to be too complex for the small amount of data under analysis. Therefore, prediction performance was quite unsatisfactory.

```{r}
doGAM <- function(train.df, pred.df, tick, response, formula, metric, xform = "",
                  orig.train.df = train.df, orig.pred.df = pred.df, orig.response = response){
    tryCatch({
        ## Write outputs to external files for later review
        par(mfrow=c(1,1))
        ...
        ## Perform training
        start <- Sys.time() #Start timer
        gam <- train(formula, data = train.df[,cols], method = "gamSpline", metric = metric,
                     trControl = ts.control, tuneGrid = tuneGridGAM, importance=TRUE)
        print(Sys.time() - start)
        ...
        if(xform == "diff"){
            ...
            plot(as.numeric(c(cumsum(c(orig.train.df[,orig.response][1], train.df[,response])),
                 ...
                 type = "l", lty = 1, xlab = "Date & Hour",
                 ylab = paste0("Cumulative Sum of ", orig.response, "[1] and DIff = ", orig.response),
                 xaxt = "n", main = paste0(tick, " gamSpline Performance (", response, "): Training + Prediction"))
            axis(1, at=1:(sum(length(train.df[,response]), length(pred.df[,response]))), labels=FALSE)
            text(1:(sum(length(train.df[,response]), length(pred.df[,response]))), par("usr")[3] - 0.2,
                 labels = c(paste(train.df$date, train.df$hour), paste(pred.df$date, pred.df$hour)),
                 srt = 90, pos = 2, xpd = TRUE, cex = 0.5, offset = -0.1)
            lines(c(cumsum(c(orig.train.df[,orig.response][1], predict(gam, train.df[,cols]))),
                    cumsum(c(orig.pred.df[,orig.response][1], predict(gam, pred.df[,cols])))),
                  type = "l", lty = 2, lwd = 2, col = "red")
            abline(v = length(train.df[,response])+1, lty = 2, col = "blue")
            ...
            plot(as.numeric(cumsum(c(orig.pred.df[,orig.response][1], pred.df[,response]))),
                 type = "l", lty = 1, xlab = "Date & Hour",
                 ylab = paste0("Cumulative Sum of ", orig.response, "[1] and DIff = ", orig.response),
                 xaxt = "n", ylim = c(min(c(cumsum(c(orig.pred.df[,orig.response][1], predict(gam, pred.df[,cols]))),
                                            cumsum(c(orig.pred.df[,orig.response][1], pred.df[,response])))),
                                      max(c(cumsum(c(orig.pred.df[,orig.response][1], predict(gam, pred.df[,cols]))),
                                            cumsum(c(orig.pred.df[,orig.response][1], pred.df[,response]))))),
                 main = paste0(tick, " gamSpline Performance (", response, "): Prediction"))
            axis(1, at=1:(length(pred.df[,response])), labels=FALSE)
            text(1:(length(pred.df[,response])), par("usr")[3] - 0.2,
                 labels = paste(pred.df$date, pred.df$hour), srt = 90, pos = 2, xpd = TRUE, cex = 0.6, offset = -0.1)
            lines(cumsum(c(orig.pred.df[,orig.response][1], predict(gam, pred.df[,cols]))),
                  type = "l", lty = 2, lwd = 2, col = "red")
        }
        ...
        plot(as.numeric(c(train.df[,response], pred.df[,response])), type = "l", lty = 1,
             xlab = "Date & Hour", ylab = response, xaxt = "n",
             main = paste0(tick, " gamSpline Performance (", response, "): Training + Prediction"))
        axis(1, at=1:(sum(length(train.df[,response]), length(pred.df[,response]))), labels=FALSE)
        text(1:(sum(length(train.df[,response]), length(pred.df[,response]))), par("usr")[3] - 0.2,
             labels = c(paste(train.df$date, train.df$hour), paste(pred.df$date, pred.df$hour)),
             srt = 90, pos = 2, xpd = TRUE, cex = 0.5, offset = -0.1)
        ...
        ## Plot just predicted performance
        plot(as.numeric(pred.df[,response]), type = "l", lty = 1, xlab = "Date & Hour", ylab = response,
             xaxt = "n", ylim = c(min(c(predict(gam, pred.df[,cols]), pred.df[,response])),
                                  max(c(predict(gam, pred.df[,cols]), pred.df[,response]))),
             main = paste0(tick, " gamSpline Performance (", response, "): Prediction"))
        axis(1, at=1:(length(pred.df[,response])), labels=FALSE)
        text(1:(length(pred.df[,response])), par("usr")[3] - 0.2, labels = paste(pred.df$date, pred.df$hour),
             srt = 90, pos = 2, xpd = TRUE, cex = 0.6, offset = -0.1)
        ...
        tryCatch({
            ...
            on.exit(close(warningOut), add = TRUE)
            on.exit(sink(), add = TRUE)
            return(list(gam, list(r2.train, r2.pred), list(mse.train, mse.pred),
                        list(rmse.train, rmse.pred), list(mae.train, mae.pred),
                        list(mape.train, mape.pred), feat.imp))
        }, error = function(e){
            on.exit(dev.off())
...
The above *doGAM* function was called via several combinations of ticker, response measure (closing price, differenced closing price, percent return, and differenced volatility), and R-squared vs. RMSE evaluation metrics, just like for RF, XGB, and kNN models. These function calls are as follows.
...
AAPL.closediff.gam.Rsquared <- doGAM(AAPL.train, AAPL.pred, "AAPL", "close.diff", close.diff ~ .,
                                     "Rsquared", "diff", AAPL.train.adder, AAPL.pred.adder, "close")
XOM.closediff.gam.Rsquared <- doGAM(XOM.train, XOM.pred, "XOM", "close.diff", close.diff ~ .,
                                    "Rsquared", "diff", XOM.train.adder, XOM.pred.adder, "close")
AAPL.closediff.gam.RMSE <- doGAM(AAPL.train, AAPL.pred, "AAPL", "close.diff", close.diff ~ .,
                                 "RMSE", "diff", AAPL.train.adder, AAPL.pred.adder, "close")
XOM.closediff.gam.RMSE <- doGAM(XOM.train, XOM.pred, "XOM", "close.diff", close.diff ~ .,
                                "RMSE", "diff", XOM.train.adder, XOM.pred.adder, "close")
...
AAPL.volatilitydiff.gam.Rsquared <- doGAM(AAPL.train, AAPL.pred, "AAPL", "volatility.diff", volatility.diff ~ .,
                                          "Rsquared", "diff", AAPL.train.adder, AAPL.pred.adder, "volatility")
XOM.volatilitydiff.gam.Rsquared <- doGAM(XOM.train, XOM.pred, "XOM", "volatility.diff", volatility.diff ~ .,
                                         "Rsquared", "diff", XOM.train.adder, XOM.pred.adder, "volatility")
AAPL.volatilitydiff.gam.RMSE <- doGAM(AAPL.train, AAPL.pred, "AAPL", "volatility.diff", volatility.diff ~ .,
                                      "RMSE", "diff", AAPL.train.adder, AAPL.pred.adder, "volatility")
XOM.volatilitydiff.gam.RMSE <- doGAM(XOM.train, XOM.pred, "XOM", "volatility.diff", volatility.diff ~ .,
                                     "RMSE", "diff", XOM.train.adder, XOM.pred.adder, "volatility")

print(Sys.time() - startOverall)
```

An example plotted output via the *doGAM* function calls is as follows. The example shown below is for the AAPL hourly closing price (To see all outputs, please refer to the *GAMplots_* PDF file outputs included with this project's submission). As these plot results clearly indicate, this model is not suitable for my sentiment and stock measure data, or at least not in such small quantity. As did the multiple linear regression model of Project 1, the GAM model performance appears to be random with extremely large prediction values and swings when predicting measures using the test dataset.

![AAPL, Closing Price, Rsquared GAM Model Evaluation Output](GAMplots_AAPLclose_Rsquared.pdf){width=800px height=800px}

Again for the Generalized Additive Models, trained models and other *doGAM* returned outputs were saved to RDS files after generation to avoid needing to re-run the code multiple times (~7 hour runtime on my PC).
...
compareMods <- function(pat, response){
    ...
    modInfo <- data.frame(model.name = character(), ticker = character(), r2.train = numeric(),
                          r2.pred = numeric(), mse.train = numeric(), mse.pred = numeric(), rmse.train = numeric(),
                          rmse.pred = numeric(), mae.train = numeric(), mae.pred = numeric(), mape.train = numeric(),
                          mape.pred = numeric())
    for(m in modList){
    ...
    ## Make final rankings
    modRanks <- data.frame(model.name, ticker, r2.train, r2.pred, mse.train, mse.pred, rmse.train, rmse.pred,
                           mae.train, mae.pred, mape.train, mape.pred)
...
The above *compareMods* function is called for each individual ticker and response type, thus creating a metrics and rank table for each.
...
Example metric and rank outputs for the AAPL/closing price ticker/response combination are shown below. Note that while the metrics table can be overwhelming, letting R do the ranking for me makes my job of working through model performance comparisons much easier. My final evaluation for the paper and presentation was performed after outputting such data for each ticker to Tableau for easier review. Note that prediction R-squared results are negative in many cases, normally suggesting very poor model performance. Due to these values being negative, I chose to ignore prediction R-squared values in my model rankings and to rely on all other metrics instead.

```{r}
Comparison.AAPL.close
Ranks.AAPL.close
```

![AAPL, Closing Price, Model Comparisons on Resampled Results](Comparison_AAPLclose.pdf){width=800px height=800px}

The above comparison and ranking results for each ticker/response combination, along with feature importance rankings, were finally output to Tableau for final model review. In all cases, all data were combined among all tickers such that only four tables were output to Tableau. A ticker column was added to support filtering each dataset by ticker using Tableau's filters. My code to generate these outputs is as follows.
